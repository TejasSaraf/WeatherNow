...
export default function WeatherDetails() {
  ...
  const validateLocation = useCallback((input) => {
    ...
    const isPostalCode = Object.values(POSTAL_CODE_FORMATS).some((format) =>
      format.test(input)
    );
    if (isPostalCode) {
      return true;
  ...
  const getWeather = useCallback(
    async (searchLocation) => {
      ...
      try {
        ...
        setLocation(response.data.current.name);
      } catch (err) {
        console.error("Weather Error:", err);
        if (err.response?.status === 404) {
          const isPostalCode = Object.values(POSTAL_CODE_FORMATS).some(
            (format) => format.test(searchLocation)
          );
          if (isPostalCode) {
            setError(
              `Postal code "${searchLocation}" not found. Please check the format and try again.`
            );
          } else {
            setError(
              `Location "${searchLocation}" not found. Please check the spelling or try a different location.`
            );
          }
        } else if (err.response?.status === 400) {
          setError(
            "Invalid location format. Please enter a valid city name, postal code, or coordinates."
          );
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        try {
          ...
          const formattedCoords = `${latitude.toFixed(6)},${longitude.toFixed(
            6
          )}`;
      ...
      (error) => {
        ...
        switch (error.code) {
          case error.PERMISSION_DENIED:
            errorMessage +=
              "Please allow location access or enter a location manually.";
            break;
          case error.POSITION_UNAVAILABLE:
            errorMessage +=
              "Location information is unavailable. Please enter a location manually.";
            break;
          case error.TIMEOUT:
  ...
  const handleUnitChange = async (newUnit) => {
    ...
    if (weather) {
      ...
      try {
        ...
        console.error("Weather Error:", err);
        setError("Failed to update temperature unit. Please try again.");
      } finally {
        setLoading(false);
      }
    }
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="text-center">
                ...
                <TemperatureDisplay
                  ...
                  unit={unit}
                />
                <p className="text-xl capitalize text-white/80">
